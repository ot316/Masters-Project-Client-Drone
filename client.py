...
import numpy, cv2, time, pickle, sys, socket, picamera, warnings
...
while True:  
    try:
        ...
        warnings.warn("Connection rejected, retrying...")
        if seconds_disconnected > disconnect_timeout:
            warnings.warn(f"No Connection for {disconnect_timeout} secondes, returning to launch")
            time.sleep(5)
...
while True:
    while True:          
        ...
        while True:
            ...
            data = data_return(output, position, vehicle.battery.voltage)
